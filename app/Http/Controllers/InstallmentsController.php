...
class InstallmentsController extends Controller
{
    ...
    public function payByAlipay(Installment $installment)
    {
        ...
        return app('alipay')->web([
            ...
            'notify_no' => 'http://requestbin.leo108.com/wsu6eaws',
            'return_no' => route('installments.alipay.return')
        ]);
    }
...
//        }
    ...
    public function alipayNotify()
    {
        ...
        // 根据分期流水号查询对应的分期记录, 原则上不会找不到, 这里的判断只是增强代码的健壮性
        if ( !$installment = Installment::where('no', $no)->first() ) {
            return 'fail';
        }
        // 根据还款计划编号查询对应的还款计划, 原则上不会找不到, 这里的判断只是增强代码的健壮性
        if ( !$item = $installment->items()->where('sequence', $sequence)->first() ) {
            return 'fail';
        }
        // 根据还款计划的支付状态是已支付, 则告知支付宝此订单已完成, 并不在执行后续逻辑
        if ( $item->paid_at ) {
            return app('alipay')->success();
        }
