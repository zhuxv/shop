...
Class SeckillProductsController extends CommonProductsController {
    ...
    protected function customForm(Form $form)
    {
        ...
        $form->saved(function (Form $form){
            ...
            // 遍历商品 SKU
            $product->skus->each(function (ProductSku $productSku) use ($diff, $product){
                // 如果秒杀商品是上架并且尚未达到结束时间
                if ( $product->on_sale && $diff > 0 ) {
                    // 将剩余库存写入到 Redis 中, 并设置该值过期时间为秒杀截止时间
                    Redis::setex('seckill_sku_'.$productSku->id, $diff, $productSku->stock);
                } else {
                    // 否则将该 SKU 的库存值从 Redis 中删除
                    Redis::del('seckill_sku_'.$productSku->id);
                }
            });
        });
    }
